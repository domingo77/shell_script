...
function generat_expect_file_about_sshfs_mount(){
    ...
    if [[ ! -f $HOME/bin/mount_remote_server_to_local ]]; then
         echo "$HOME/bin/mount_remote_server_to_local not exist, so create it"
         echo "#!/usr/bin/expect"                        > $HOME/bin/mount_remote_server_to_local
         echo "log_user 1"                               >> $HOME/bin/mount_remote_server_to_local
         echo "set user [lindex \$argv 0]"                >> $HOME/bin/mount_remote_server_to_local
         echo "set host [lindex \$argv 1]"                       >> $HOME/bin/mount_remote_server_to_local
         echo "set remote_dir [lindex \$argv 2]"                  >> $HOME/bin/mount_remote_server_to_local
         echo "set mount_directory_path [lindex \$argv 3]"        >> $HOME/bin/mount_remote_server_to_local
         echo "set option [lindex \$argv 4]"                      >> $HOME/bin/mount_remote_server_to_local
         echo "set option_parameter [lindex \$argv 5]"            >> $HOME/bin/mount_remote_server_to_local
         echo "spawn -ignore HUP sshfs \$user@\$host:\$remote_dir   \$mount_directory_path -\$option \$option_parameter"          >> $HOME/bin/mount_remote_server_to_local
         echo "#spawn -ignore HUP sshfs root@atgasia01:/var/www/html /home/mero/remote-root-atgasia01-1082 -p 1082"         >> $HOME/bin/mount_remote_server_to_local
         echo "sleep 2"                                         >> $HOME/bin/mount_remote_server_to_local
         echo "expect \"*password:\""                             >> $HOME/bin/mount_remote_server_to_local
         echo "send \"mediapool\\r\""                            >> $HOME/bin/mount_remote_server_to_local
         echo "interact"                                               >> $HOME/bin/mount_remote_server_to_local  
         chmod 0777 $HOME/bin/mount_remote_server_to_local
         if [[ -f $HOME/bin/mount_remote_server_to_local ]]; then
             echo "Successfully create expect file : $HOME/bin/mount_remote_server_to_local, for automatically mount remote server to local"
         else
             echo "Error: Failed to create expect file : $HOME/bin/mount_remote_server_to_local"
	     exit -1
         fi
    else
         echo "$HOME/bin/mount_remote_server_to_local exist"
    fi
...
function check_remote_server_is_mounted_to_local_directory(){
    ...
    if [[ ! -d $HOME/remote-root-atgasia01-1082 ]]; then
         echo "$HOME/remote-root-atgasia01-1082 not exist, so create it"
         mkdir $HOME/remote-root-atgasia01-1082
	 ls $HOME/remote-root-atgasia01-1082
    fi
    if [[ -d $HOME/remote-root-atgasia01-1082 ]]; then
        echo "$HOME/remote-root-atgasia01-1082 exist,so first unmount reomote server"
        fusermount -u $HOME/remote-root-atgasia01-1082
	mount_remote_server_to_local root atgasia01 "/var/www/html" "$HOME/remote-root-atgasia01-1082" p 1082
	if [[ -f $HOME/remote-root-atgasia01-1082/_MEDIA_POOL_ ]]; then
            echo "Successfully Mount to local directory $HOME/remote-root-atgasia01-1082/"
	    ls -lh $HOME/remote-root-atgasia01-1082/
        else
...
function check_record_xml_exist(){
    ...
    if [[ ! -f $HOME/Videos/Test_stream_summary.xml ]]; then
        cp -rf $HOME/1-never-delete/Saved-Test_streams_template-modified-ubuntu.xml $HOME/Videos/Test_stream_summary.xml 
    fi
...
function check_record_xml_with_file_size_exist(){
    ...
    if [[ ! -f $HOME/Videos/Test_stream_summary.xml ]]; then
        cp -rf $HOME/1-never-delete/Backup.xml $HOME/Videos/Test_stream_summary.xml 
    fi
...
function check_remote_space_is_enough(){
    ...
    echo -e "###############################"
    cat $HOME/1-never-delete/remote_space.txt
    remote_avail_space=`cat $HOME/1-never-delete/remote_space.txt `
    ...
    echo "$remain_remote_avail_space" > $HOME/1-never-delete/remote_space.txt
    cat $HOME/1-never-delete/remote_space.txt
    echo -e "##############################"
...
function check_file_name_is_valid(){
    ...
    cat ls.txt
    echo -e "\n>>>>>>>>> begin <<<<<<<<<<\n"
    echo -e  "\n\n[Stream Directory path: "$1" count: $files_count]" >> "$HOME/Videos/$step_name_suffix/Step1-Check_name_is_valid.txt" 
    while read line
    do
    echo "#########################################################################"
    echo "$line"
    ...
    if [[ ! -z $local_space ]] || [[ ! -z $local_left_bracket ]] \
       ...
       || [[ ! -z $local_minus ]];then
        ...
        if [[ -f $line ]]; then
            ...
            if [[ ! -z $check_exist ]]; then
                ...
                if [[ $? == 0 ]]; then 
                    ...
                    echo -e  "[Invalid][Success] "$tmp" ---> "SRDCQA-$count-$line"" >> "$HOME/Videos/$step_name_suffix/Step1-Check_name_is_valid.txt" 
                else 
                    ...
                    echo "END exit -1"
                    rm -rf ls.txt
                    echo -e  "[Invalid][Failure] "$tmp" ---> "SRDCQA-$count-$line"" >> "$HOME/Videos/$step_name_suffix/Step1-Check_name_is_valid.txt" 
                    echo "ERROR: file name is invalid.please check key [Invalid][Failure] in $HOME/Videos/$step_name_suffix/Step1-Check_name_is_valid.txt"
                    echo "######################Failure: file_name_is_invalid"
                    echo "######################please contact Ming.Dong, let him change the file name manually"
                    echo "`cat "$HOME/Videos/$step_name_suffix/Step1-Check_name_is_valid.txt" | grep -i   "[Invalid][Failure]"`"
                    echo -e "\n\n"
                    exit -1
                fi
            else 
                ...
                if [[ $? == 0 ]]; then 
                    ...
                    echo -e  "[Invalid][Success] "$tmp" ---> "SRDCQA-1-$line"" >> "$HOME/Videos/$step_name_suffix/Step1-Check_name_is_valid.txt" 
                else 
                    echo "Failed to modify name"
                    echo "######################################################################"
                    echo "BEGIN $tmp"
                    echo "END exit -1"
                    rm -rf ls.txt					
                    echo -e  "[Invalid][Failure] "$tmp" ---> "SRDCQA-1-$line"" >> "$HOME/Videos/$step_name_suffix/Step1-Check_name_is_valid.txt" 
                    echo "ERROR: file name is invalid.please check key [Invalid][Failure] in $HOME/Videos/$step_name_suffix/Step1-Check_name_is_valid.txt"
                    echo "######################Failure: file_name_is_invalid"
                    echo "######################please contact Ming.Dong, let him change the file name manually"
                    echo "`cat "$HOME/Videos/$step_name_suffix/Step1-Check_name_is_valid.txt" | grep -i   "[Invalid][Failure]"`"
            ...
            mv ./"$tmp" $line
                if [[ $? == 0 ]]; then 
                    ...
                    echo "BEGIN $tmp"
                    echo "END $line"
                    echo -e  "[Invalid][Success] "$tmp" ---> "$line"" >> "$HOME/Videos/$step_name_suffix/Step1-Check_name_is_valid.txt" 
                else 
                    echo "Failed to modify name"
                    echo "######################################################################"
                    echo "BEGIN $tmp"
                    echo "END exit -1"
                    echo -e  "[Invalid][Failure] "$tmp" ---> "$line"" >> "$HOME/Videos/$step_name_suffix/Step1-Check_name_is_valid.txt" 
                    rm -rf ls.txt					
                    echo "ERROR: file name is invalid.please check key [Invalid][Failure] in $HOME/Videos/$step_name_suffix/Step1-Check_name_is_valid.txt"
                    echo "######################Failure: file_name_is_invalid"
                    echo "######################please contact Ming.Dong, let him change the file name manually"
        ...
        echo "BEGIN $tmp"
        echo "END $line"
        echo -e  "[Vaild] "$tmp" ---> "$line"" >> "$HOME/Videos/$step_name_suffix/Step1-Check_name_is_valid.txt" 
    fi 
    echo -e "\n\n"
    done < ls.txt
...
function check_same_file_in_xml(){
    old_name_exist=`cat "$HOME/Videos/Test_stream_summary.xml" | grep ">$1<"`
    echo "old_name_exist=$old_name_exist"
    if [[ ! -z $old_name_exist ]]; then
        new_name_exist=`cat "$HOME/Videos/Test_stream_summary.xml" | grep ">$1<" -A 18 | grep ">$2<"`
        current_file_size=`ls -l "$1" | awk -F " " '{print $5}'`
        same_size_exist=`cat "$HOME/Videos/Test_stream_summary.xml" | grep ">$1<" -A 18 | grep "$current_file_size"`
	...
	if [[ ! -z $new_name_exist ]] && [[ ! -z $same_size_exist ]]; then
            continue_flag=true
	    echo "WARNING: origin:$1---final:$2---size:$current_file_size already in $HOME/Videos/Test_stream_summary.xml"
	    echo "Continue"
	    if [[ ! -d $HOME/Videos/Already_exist_in_xml ]]; then
                mkdir -p $HOME/Videos/Already_exist_in_xml
            fi
            cp -rf "$a" $HOME/Videos/Already_exist_in_xml/"$a"
            echo -e "[$3][Already Exist][origin:$1][final:$2][size:${current_file_size}B]"  >> $HOME/Videos/$step_name_suffix/Step4-steam_exist_in_xml.txt
        fi
    else
        echo "$2"
        new_name_exist=`cat "$HOME/Videos/Test_stream_summary.xml" | grep ">$2<"`
        current_file_size=`ls -l "$1" | awk -F " " '{print $5}'`
        same_size_exist=`cat "$HOME/Videos/Test_stream_summary.xml" | grep ">$2<" -A 17 | grep ">${current_file_size}B<"`
        echo "new_name_exist=$new_name_exist"
        echo "current_file_size=$current_file_size"
        echo "same_size_exist=$same_size_exist"
	if [[ ! -z $new_name_exist ]] && [[ ! -z $same_size_exist ]]; then
            origin_name_in_xml=`cat "$HOME/Videos/Test_stream_summary.xml" | grep ">$2<" -B 1 | head -n 1 | awk -F ">" '{print $3}' | awk -F "<" '{print $1}'`
            continue_flag=true
	    echo "WARNING: final:$2---size:$current_file_size already in $HOME/Videos/Test_stream_summary.xml"
	    echo "Continue"
	    if [[ ! -d $HOME/Videos/Already_exist_in_xml ]]; then
                mkdir -p $HOME/Videos/Already_exist_in_xml
            fi
            cp -rf "$a" $HOME/Videos/Already_exist_in_xml/"$a"
            echo -e "[$3][Already Exist][origin_in_xml:$origin_name_in_xml][origin:$1][final:$2][size:${current_file_size}B]"  >> $HOME/Videos/$step_name_suffix/Step4-steam_exist_in_xml.txt
        fi
    fi
}
...
function create_local_video_format_directory_and_move(){
    ...
    echo -e "###############################"
        ...
        if [[ ! -z $classification_directory_name ]]; then
           ...
           #echo "[$1] : "$a" ------> "${final_name_as_per_name_rule}""  >> $HOME/Videos/$step_name_suffix/Step5-old-new-one-to-one.txt
           ...
           if [[ -d $HOME/Videos/$classification_directory_name ]]; then
               if [[ ! -z $display_resolution ]]; then
                   ...
                   if [[ -d $HOME/Videos/$classification_directory_name/$display_resolution ]]; then
                       echo "check whether the same final name ${final_name_as_per_name_rule} is in diectory $HOME/Videos/$classification_directory_name/$display_resolution"
		       if [[ -f $HOME/Videos/$classification_directory_name/${display_resolution}/"${final_name_as_per_name_rule}" ]]; then
                           check_final_exist=`ls $HOME/Videos/$classification_directory_name/${display_resolution}/${final_name_as_per_name_rule_prefix}* | grep -i "SRDCQA-" `
                           ...
                           if [[ ! -z $check_final_exist ]]; then
                               ...
                               count=`ls $HOME/Videos/$classification_directory_name/${display_resolution}/${final_name_as_per_name_rule_prefix}* | grep -i "SRDCQA-" | awk -F "SRDCQA-" '{print $2}' | awk -F "." '{print $1}' | sort -n | tail -1`
                               echo "count=$count"
                               let count+=1
                               echo "count=$count"
                               cp -rf "$a" $HOME/Videos/$classification_directory_name/${display_resolution}/"${final_name_as_per_name_rule_prefix}"-SRDCQA-$count."${final_name_as_per_name_rule_suffix}"
                               echo "[$1][Success] : "$local_stream_path"/"$a" ------> $HOME/Videos/$classification_directory_name/$display_resolution/"${final_name_as_per_name_rule_prefix}"-SRDCQA-$count."${final_name_as_per_name_rule_suffix}""  >> $HOME/Videos/$step_name_suffix/Step5-old-new-one-to-one.txt
			       final_name_as_per_name_rule="${final_name_as_per_name_rule_prefix}"-SRDCQA-$count."${final_name_as_per_name_rule_suffix}"
                           else
                               cp -rf "$a" $HOME/Videos/$classification_directory_name/${display_resolution}/"${final_name_as_per_name_rule_prefix}"-SRDCQA-1."${final_name_as_per_name_rule_suffix}"
                               echo "[$1][Success] : "$local_stream_path"/"$a" ------> $HOME/Videos/$classification_directory_name/$display_resolution/"${final_name_as_per_name_rule_prefix}"-SRDCQA-1."${final_name_as_per_name_rule_suffix}""  >> $HOME/Videos/$step_name_suffix/Step5-old-new-one-to-one.txt
			       final_name_as_per_name_rule="${final_name_as_per_name_rule_prefix}"-SRDCQA-1."${final_name_as_per_name_rule_suffix}"
                           ...
                           echo "[$1][Success] : "$local_stream_path"/"$a" ------> $HOME/Videos/$classification_directory_name/$display_resolution/"${final_name_as_per_name_rule}""  >> $HOME/Videos/$step_name_suffix/Step5-old-new-one-to-one.txt
                       fi
                   fi
               else # no match display resolution
                   continue_flag=true
                   echo "Not match display resolution, so enter directory: $HOME/Videos/$classification_directory_namei/Unidentified_resolution, and copy"
                   if [[ ! -d $HOME/Videos/$classification_directory_name/Unidentified_resolution ]]; then
                       mkdir -p $HOME/Videos/$classification_directory_namei/Unidentified_resolution
                   fi
                   cp -rf "$a" $HOME/Videos/$classification_directory_name/Unidentified_resolution/"$a"
                   width_height_pixel=`echo $name_part3_part4_local_video_width_height | sed -e "s/^_//g"`
                   echo "[$1][Failure][width_height_pixel:$width_height_pixel] : "$local_stream_path"/"$a" ------> $HOME/Videos/$classification_directory_name/Unidentified_resolution/"$a"--->final name:"${final_name_as_per_name_rule}""  >> $HOME/Videos/$step_name_suffix/Step5-old-new-one-to-one.txt
                   echo "[$1][Failure][width_height_pixel:$width_height_pixel] : "$local_stream_path"/"$a" ------> $HOME/Videos/$classification_directory_name/Unidentified_resolution/"$a"--->final name:"${final_name_as_per_name_rule}""  >> $HOME/Videos/$step_name_suffix/Step3-Unidentified_resolution.txt
               fi # check $display_resolution
           fi # check $HOME/Videos/$classification_directory_name
...
function create_remote_video_format_directory_and_move(){
	...
	if [[ -f $HOME/remote-root-atgasia01-1082/_MEDIA_POOL_ ]]; then
        ...
        if [[ ! -z $classification_directory_name ]]; then
           if [[ ! -d $HOME/remote-root-atgasia01-1082/$classification_directory_name/ ]]; then
             mkdir -p $HOME/remote-root-atgasia01-1082/$classification_directory_name/
           fi
           #echo "[$1] : "$a" ------> "${final_name_as_per_name_rule}""  >> $HOME/Videos/$step_name_suffix/Step5-old-new-one-to-one.txt
           echo "classification_directory_name=[$classification_directory_name]"
           if [[ -d $HOME/remote-root-atgasia01-1082/$classification_directory_name ]]; then
               if [[ ! -z $display_resolution ]]; then
                   if [[ ! -d $HOME/remote-root-atgasia01-1082/$classification_directory_name/$display_resolution ]]; then
                       mkdir -p $HOME/remote-root-atgasia01-1082/$classification_directory_name/$display_resolution
                   fi
                   if [[ -d $HOME/remote-root-atgasia01-1082/$classification_directory_name/$display_resolution ]]; then
                       echo "check whether the same final name ${final_name_as_per_name_rule} is in diectory $HOME/remote-root-atgasia01-1082/$classification_directory_name/$display_resolution"
		       if [[ -f $HOME/remote-root-atgasia01-1082/$classification_directory_name/${display_resolution}/"${final_name_as_per_name_rule}" ]]; then
                           check_final_exist=`ls $HOME/remote-root-atgasia01-1082/$classification_directory_name/${display_resolution}/${final_name_as_per_name_rule_prefix}* | grep -i "SRDCQA-" `
                           echo "check_final_exist=$check_final_exist"
                           if [[ ! -z $check_final_exist ]]; then
                               echo "There is a suffix "SRDCQA-n",so change to "SRDCQA-n+1""
                               count=`ls $HOME/remote-root-atgasia01-1082/$classification_directory_name/${display_resolution}/${final_name_as_per_name_rule_prefix}* | grep -i "SRDCQA-" | awk -F "SRDCQA-" '{print $2}' | awk -F "." '{print $1}' | sort -n | tail -1`
                               echo "count=$count"
                               let count+=1
                               echo "count=$count"
                               cp -rf "$a" $HOME/remote-root-atgasia01-1082/$classification_directory_name/${display_resolution}/"${final_name_as_per_name_rule_prefix}"-SRDCQA-$count."${final_name_as_per_name_rule_suffix}"
                               echo "[$1][Success] : "$local_stream_path"/"$a" ------> $HOME/remote-root-atgasia01-1082/$classification_directory_name/$display_resolution/"${final_name_as_per_name_rule_prefix}"-SRDCQA-$count."${final_name_as_per_name_rule_suffix}""  >> $HOME/remote-root-atgasia01-1082/$step_name_suffix/Step5-old-new-one-to-one.txt
			       final_name_as_per_name_rule="${final_name_as_per_name_rule_prefix}"-SRDCQA-$count."${final_name_as_per_name_rule_suffix}"
                           else
                               cp -rf "$a" $HOME/remote-root-atgasia01-1082/$classification_directory_name/${display_resolution}/"${final_name_as_per_name_rule_prefix}"-SRDCQA-1."${final_name_as_per_name_rule_suffix}"
                               echo "[$1][Success] : "$local_stream_path"/"$a" ------> $HOME/remote-root-atgasia01-1082/$classification_directory_name/$display_resolution/"${final_name_as_per_name_rule_prefix}"-SRDCQA-1."${final_name_as_per_name_rule_suffix}""  >> $HOME/remote-root-atgasia01-1082/$step_name_suffix/Step5-old-new-one-to-one.txt
			       final_name_as_per_name_rule="${final_name_as_per_name_rule_prefix}"-SRDCQA-1."${final_name_as_per_name_rule_suffix}"
                           fi
                           
                       else
                           cp -rf "$a" $HOME/remote-root-atgasia01-1082/$classification_directory_name/${display_resolution}/"${final_name_as_per_name_rule}"
                           echo "[$1][Success] : "$local_stream_path"/"$a" ------> $HOME/remote-root-atgasia01-1082/$classification_directory_name/$display_resolution/"${final_name_as_per_name_rule}""  >> $HOME/remote-root-atgasia01-1082/$step_name_suffix/Step5-old-new-one-to-one.txt
                       fi
                   fi
               else # no match display resolution
                   continue_flag=true
...
# prerequisit: check whether  mount_remote_server_to_local exist.
generat_expect_file_about_sshfs_mount

# prerequisite 1: whether remote server is mounted to local directory remote-root-atgasia01-1082\
...
if [[ ! -d $HOME/Videos/$step_name_suffix ]]; then
    mkdir $HOME/Videos/$step_name_suffix
fi
# generate record file
files_count=`ls |wc -l`
echo -e "\n\n[Stream Directory path: "$1" count: $files_count]" >> "$HOME/Videos/$step_name_suffix/Step5-old-new-one-to-one.txt" 
echo -e "\n\n[Stream Directory path: "$1" count: $files_count]"  >> $HOME/Videos/$step_name_suffix/Step2-Unidentified_track_type.txt
echo -e "\n\n[Stream Directory path: "$1" count: $files_count]"  >> $HOME/Videos/$step_name_suffix/Step3-Unidentified_resolution.txt
echo -e "\n\n[Stream Directory path: "$1" count: $files_count]"  >> $HOME/Videos/$step_name_suffix/Step4-steam_exist_in_xml.txt
...
do
    ...
    if [[ -z $only_video ]] && [[ -z $video_audio ]] && [[ -z $video_audio1_audio2 ]]; then
        echo "[$count][Erorr] : "$a"  Error: track type mismatch, please check  "  >> $HOME/Videos/$step_name_suffix/Step5-old-new-one-to-one.txt
        echo "[$count][Erorr] : "$a"  Error: track type mismatch, please check  "  >> $HOME/Videos/$step_name_suffix/Step2-Unidentified_track_type.txt
        mediainfo -f --Output=XML "$a" | grep -i "track type" >> $HOME/Videos/$step_name_suffix/Step2-Unidentified_track_type.txt
	if [[ ! -d $HOME/Videos/Unidentified_track_type ]]; then
            mkdir $HOME/Videos/Unidentified_track_type
        fi
	cp -rf "$a" $HOME/Videos/Unidentified_track_type/"$a"
    ...
    if [[ ! -z $only_video ]]; then
        #create_row_template "$HOME/Videos/Test_stream_summary.xml"
	create_row_template_with_file_size "$HOME/Videos/Test_stream_summary.xml"
        add_parsed_parameters_into_row_template_for_only_video "$HOME/Videos/Test_stream_summary.xml" "$a"
    fi
     
    if [[ ! -z $video_audio ]]; then
        #create_row_template "$HOME/Videos/Test_stream_summary.xml"
	create_row_template_with_file_size "$HOME/Videos/Test_stream_summary.xml"
        add_parsed_parameters_into_row_template_for_video_audio "$HOME/Videos/Test_stream_summary.xml"   "$a"
    ...
    if [[ ! -z $video_audio1_audio2 ]]; then
        #create_row_template "$HOME/Videos/Test_stream_summary.xml"
	create_row_template_with_file_size "$HOME/Videos/Test_stream_summary.xml" 
        add_parsed_parameters_into_row_template_for_video_audio1_audio2 "$HOME/Videos/Test_stream_summary.xml" "$a" 
...
Unidentified_track_type=`cat $HOME/Videos/$step_name_suffix/Step2-Unidentified_track_type.txt  | grep -i   "Error: track type mismatch, please check"`
if [[ ! -z $Unidentified_track_type ]]; then
    echo "######################Failure: Unidentified_track_type "
    echo "######################please contact Ming.Dong, let him check"
    echo "`cat $HOME/Videos/$step_name_suffix/Step2-Unidentified_track_type.txt  | grep -i   "Error: track type mismatch, please check"`"
    echo -e "\n\n"
fi

Unidentified_resolution=`cat $HOME/Videos/$step_name_suffix/Step3-Unidentified_resolution.txt  | grep -i   "\[Failure\]\[width_height_pixel:"`
if [[ ! -z $Unidentified_resolution ]]; then
    echo "######################Failure: Unidentified_resolution "
    echo "######################please contact Ming.Dong, let him add the corresponding width_height_pixel"
    echo "`cat $HOME/Videos/$step_name_suffix/Step3-Unidentified_resolution.txt  | grep -i   "[Failure][width_height_pixel:"`"
    echo -e "\n\n"
fi

Already_exist_in_xml=`cat $HOME/Videos/$step_name_suffix/Step4-steam_exist_in_xml.txt  | grep -i   "Already Exist"`
if [[ ! -z $Already_exist_in_xml ]]; then
    echo "######################WARNING: Already_exist_in_xml "
    echo "######################please contact Ming.Dong, let him check"
    echo "`cat $HOME/Videos/$step_name_suffix/Step4-steam_exist_in_xml.txt  | grep -i   "Already Exist"`"
    echo -e "\n\n"
fi
...
if [[ `cat $HOME/1-never-delete/remote_space.txt` -le 5242880 ]]; then 
