...
do
    ...
    if [[ -z $only_video ]] && [[ -z $video_audio ]] && [[ -z $video_audio1_audio2 ]]; then
        ...
        mediainfo -f --Output=XML "$a" | grep -i "track type" >> $HOME/$local_saved_directory/$step_name_suffix/Step2-Unidentified_track_type.txt
	...
	create_row_template_with_file_size "$HOME/$local_saved_directory/$summary_xml_name"
