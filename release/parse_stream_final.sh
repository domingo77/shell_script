...
function install_necessary_packages(){
    ...
    if [[ -z ${expect_package_install} ]] || [[ -z ${openssh_server_package_install} ]] || [[ -z ${mediainfo_package_install} ]] \
        ...
        || [[ -z ${net_tools_pacakge_install} ]] || [[ -z ${vim_pacakge_install} ]]; then
...
if [[ ! -f $HOME/bin/upload_stream_final.sh ]] || [[ ! -f $HOME/1-never-delete/Backup.xml ]] || [[ ! -f $HOME/1-never-delete/remote_space.txt ]]; then
...
upload_stream_final.sh $1 2>&1 | tee $HOME/logs/$log_name_suffix-$tmp_date.txt
if [[  ! -f $HOME/bin/parse_stream_final.sh ]]; then
    cp -rf ~/parse_stream_final.sh $HOME/bin/parse_stream_final.sh
    chmod 0777 $HOME/bin/parse_stream_final.sh
    ls $HOME/bin/parse_stream_final.sh
fi
